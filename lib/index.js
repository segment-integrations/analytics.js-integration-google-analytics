...
GA.prototype.initialize = function() {
  ...
  /**
   * IMPORTANT: ensure you are aware of this conditional when testing.
   * This will hard code a value of 'none' as the cookie domain name when running on localhost.
   * In production if this is not set, the default will be an empty string which will resolve as falsey.
  ...
  var config = {
    ...
    cookieDomain: opts.domain || this.defaults.domain,
  ...
  if (opts.optimize) window.ga(this._getTrackerName() + 'require', opts.optimize);

  // display advertising
  if (opts.doubleClick) {
    window.ga(this._getTrackerName() + 'require', 'displayfeatures');
  }

  // https://support.google.com/analytics/answer/2558867?hl=en
  if (opts.enhancedLinkAttribution) {
    window.ga(this._getTrackerName() + 'require', 'linkid', 'linkid.js');
  }

  // send global id
  if (opts.sendUserId && user.id()) {
    window.ga(this._getTrackerName() + 'set', 'userId', user.id());
  }

  // anonymize after initializing, otherwise a warning is shown
  // in google analytics debugger
  if (opts.anonymizeIp) window.ga(this._getTrackerName() + 'set', 'anonymizeIp', true);
  ...
  if (len(custom)) window.ga(this._getTrackerName() + 'set', custom);
...
GA.prototype.page = function(page) {
  ...
  if (len(custom)) {
    if (opts.setAllMappedProps) {
      window.ga(this._getTrackerName() + 'set', custom);
    } else {
  ...
  window.ga(this._getTrackerName() + 'set', payload);

  if (this.pageCalled) delete pageview.location;
  
  // send
  window.ga(this._getTrackerName() + 'send', 'pageview', pageview);
...
GA.prototype.identify = function(identify) {
  ...
  if (opts.sendUserId && identify.userId()) {
    window.ga(this._getTrackerName() + 'set', 'userId', identify.userId());
  }
...
GA.prototype.track = function(track, options) {
  ...
  window.ga(this._getTrackerName() + 'send', 'event', payload);
};
...
GA.prototype.orderCompleted = function(track) {
  ...
  // require ecommerce
  if (!this.ecommerce) {
    window.ga(this._getTrackerName() + 'require', 'ecommerce');
    this.ecommerce = true;
  }

  // add transaction
  window.ga(this._getTrackerName() + 'ecommerce:addTransaction', {
  ...
  each(products, function(product) {
    ...
    window.ga(self._getTrackerName() + 'ecommerce:addItem', {
  ...
  // send
  window.ga(this._getTrackerName() + 'ecommerce:send');
};
...
GA.prototype.loadEnhancedEcommerce = function(track) {
  ...
  if (!this.enhancedEcommerceLoaded) {
    window.ga(self._getTrackerName() + 'require', 'ec');
    this.enhancedEcommerceLoaded = true;
  }

  // Ensure we set currency for every hit
  window.ga(self._getTrackerName() + 'set', '&cu', track.currency());
};
...
GA.prototype.checkoutStepViewedEnhanced = function(track) {
  ...
  each(products, function(product) {
    var productTrack = createProductTrack(track, product);
    enhancedEcommerceTrackProduct(productTrack, self._getTrackerName());
  });
  window.ga(self._getTrackerName() + 'ec:setAction', 'checkout', {
    step: props.step || 1,
...
GA.prototype.checkoutStepCompletedEnhanced = function(track) {
  ...
  window.ga(self._getTrackerName() + 'ec:setAction', 'checkout_option', {
    step: props.step || 1,
    option: options
  });

  window.ga(self._getTrackerName() + 'send', 'event', 'Checkout', 'Option');
};
...
GA.prototype.orderCompletedEnhanced = function(track) {
  ...
  window.ga(self._getTrackerName() + 'ec:setAction', 'purchase', {
    id: orderId,
    affiliation: props.affiliation,
    revenue: total,
    tax: track.tax(),
...
GA.prototype.orderRefundedEnhanced = function(track) {
  ...
  each(products, function(product) {
    var track = new Track({ properties: product });
    window.ga(self._getTrackerName() + 'ec:addProduct', {
      id: track.productId() || track.id() || track.sku(),
      quantity: track.quantity()
    });
  });

  window.ga(self._getTrackerName() + 'ec:setAction', 'refund', {
    id: orderId
  });
...
GA.prototype.productAddedEnhanced = function(track) {
  ...
  this.loadEnhancedEcommerce(track);
  enhancedEcommerceProductAction(track, 'add', null, self._getTrackerName());
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.productRemovedEnhanced = function(track) {
  ...
  this.loadEnhancedEcommerce(track);
  enhancedEcommerceProductAction(track, 'remove', null, self._getTrackerName());
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.productViewedEnhanced = function(track) {
  ...
  enhancedEcommerceProductAction(track, 'detail', data, self._getTrackerName());
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.productClickedEnhanced = function(track) {
  ...
  // list property is optional
  if (props.list) data.list = props.list;
  enhancedEcommerceProductAction(track, 'click', data, self._getTrackerName());
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.promotionViewedEnhanced = function(track) {
  ...
  this.loadEnhancedEcommerce(track);
  window.ga(self._getTrackerName() + 'ec:addPromo', {
...
GA.prototype.promotionClickedEnhanced = function(track) {
  ...
  window.ga(self._getTrackerName() + 'ec:setAction', 'promo_click', {});
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.productListViewedEnhanced = function(track) {
  ...
  each(products, function(product) {
    ...
    window.ga(self._getTrackerName() + 'ec:addImpression', impressionObj);
  });
...
// Setting up a getter for `trackerName` to provide for easier testing.
GA.prototype._getTrackerName = function() { return this._trackerName; };
