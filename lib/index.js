...
GA.prototype.productListViewedEnhanced = function(track) {
  ...
  each(products, function(product) {
    ...
    if (!(product.product_id || product.sku) && !product.name) return;
    var impressionObj = {
      ...
      position: products.map(function(x) { return x.product_id; }).indexOf(product.product_id) + 1
    };
...
GA.prototype.productListFilteredEnhanced = function(track) {  
  ...
  var sorters = props.sorters.map(function(obj) { return obj.type + ':' + obj.value;}).join();
  this.loadEnhancedEcommerce(track);
  each(products, function(product) {
    if (!(product.product_id || product.sku) && !product.name) return;
    var impressionObj = {
