...
GA.prototype.track = function(track, options) {
  ...
  // custom dimensions & metrics
  var customDimensions = dimensions(props, interfaceOpts);
  console.log('custom variable', customDimensions);
  if (len(customDimensions)) window.ga('set', customDimensions);
  ...
  var customMetrics = metrics2(props, interfaceOpts);
  console.log('customMetrics', customMetrics);
  if (len(customMetrics)) extend(payload, customMetrics);
...
function metrics2(obj, data) {
  var metrics = data.metrics;
  var names = keys(metrics);
  var ret = {};

  for (var i = 0; i < names.length; ++i) {
    var name = names[i];
    var key = metrics[name];
    var value = dot(obj, name) || obj[name];
    if (value == null) continue;
    ret[key] = value;
  }
