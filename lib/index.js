...
GA.prototype.initialize = function() {
  ...
  // tracker name must be prepended to all ga method calls
  if (opts.nameTracker) {
    window.ga('create', opts.trackingId, config, 'segmentGATracker');
...
GA.prototype.track = function(track, options) {
  ...
  if (len(custom)) {
    if (interfaceOpts.setAllMappedProps) {
      window.ga(self._trackerName + 'set', custom);
    } else {
  ...
  window.ga(self._trackerName + 'send', 'event', payload);
};
...
GA.prototype.orderCompleted = function(track) {
  ...
  // require ecommerce
  if (!this.ecommerce) {
    window.ga(self._trackerName + 'require', 'ecommerce');
    this.ecommerce = true;
  }

  // add transaction
  window.ga(self._trackerName + 'ecommerce:addTransaction', {
  ...
  // send
  window.ga(self._trackerName + 'ecommerce:send');
};
...
GA.prototype.checkoutStepViewedEnhanced = function(track) {
  ...
  each(products, function(product) {
    var productTrack = createProductTrack(track, product);
    enhancedEcommerceTrackProduct(productTrack, self);
  });
...
GA.prototype.productAddedEnhanced = function(track) {
  ...
  this.loadEnhancedEcommerce(track);
  enhancedEcommerceProductAction(track, 'add', null, self);
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.productRemovedEnhanced = function(track) {
  ...
  this.loadEnhancedEcommerce(track);
  enhancedEcommerceProductAction(track, 'remove', null, self);
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.productViewedEnhanced = function(track) {
  ...
  enhancedEcommerceProductAction(track, 'detail', data, self);
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.productClickedEnhanced = function(track) {
  ...
  // list property is optional
  if (props.list) data.list = props.list;
  enhancedEcommerceProductAction(track, 'click', data, self);
  this.pushEnhancedEcommerce(track);
};
...
function enhancedEcommerceTrackProduct(track, context) {
  var props = track.properties();
  var product = {
    id: track.productId() || track.id() || track.sku(),
    name: track.name(),
  ...
  window.ga(context._trackerName + 'ec:addProduct', product);
}
...
function enhancedEcommerceProductAction(track, action, data, context) {
  enhancedEcommerceTrackProduct(track, context);
  window.ga(context._trackerName + 'ec:setAction', action, data || {});
}
